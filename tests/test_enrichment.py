...
class TestNormalize:
    ...
    def test_removes_year(self) -> None:
        from app.enrichment import _normalize
...
class TestNDEnrichment:
    ...
    def test_init_without_movies(self) -> None:
        from app.enrichment import NDEnrichment

        enrich = NDEnrichment()
        assert not enrich.is_loaded

    def test_init_with_movies(self) -> None:
        from app.enrichment import NDEnrichment
    ...
    def test_get_metadata_returns_none_for_unknown(self) -> None:
        from app.enrichment import NDEnrichment

        enrich = NDEnrichment()
        assert enrich.get_metadata(999999) is None

    def test_get_cast_returns_none_for_unknown(self) -> None:
        from app.enrichment import NDEnrichment

        enrich = NDEnrichment()
        assert enrich.get_cast(999999) is None

    def test_get_reviews_returns_none_for_unknown(self) -> None:
        from app.enrichment import NDEnrichment

        enrich = NDEnrichment()
        assert enrich.get_reviews(999999) is None

    def test_get_movies_by_director_returns_empty(self) -> None:
        from app.enrichment import NDEnrichment

        enrich = NDEnrichment()
        assert enrich.get_movies_by_director("Unknown Director") == []

    def test_get_movies_by_actor_returns_empty(self) -> None:
        from app.enrichment import NDEnrichment

        enrich = NDEnrichment()
        assert enrich.get_movies_by_actor("Unknown Actor") == []

    def test_has_data_false_for_empty(self) -> None:
        from app.enrichment import NDEnrichment
    ...
    def test_get_status_summary_returns_dict(self) -> None:
        from app.enrichment import NDEnrichment
